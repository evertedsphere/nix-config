...
let colors = config.colorScheme.colors; 
  ...
  renderTemplates = renderTemplate: data:
    let
      attrsToList = attrSet:
...
programs.wofi.enable = true;
  ...
  home.stateVersion = "22.11";
    ...
    services.screen-locker = {
      enable = true;
      lockCmd =
        "${pkgs.i3lock}/bin/i3lock -n -i ${config.home.file.wallpaper-folder.target}";
    };
      ...
      windowManager.i3 = {
        ...
        config =
          let
            ...
            workspaceKeybinds = renderTemplates
              ({ wsName, wsKey }: {
                "${modifier}+${wsKey}" = "workspace number ${wsName}";
                "${modifier}+Shift+${wsKey}" =
                  "move container to workspace number ${wsName}";
              })
              (smallMonitorWss ++ bigMonitorWss);
            containerKeybinds = renderTemplates
              ({ key, dir }: {
                "${modifier}+${key}" = "focus ${dir}";
                "${modifier}+Shift+${key}" = "move ${dir}";
              })
              movementKeys;
            assignWorkspace = outputs:
              let output = lib.concatStringsSep " " outputs;
              in
              map (workspace: {
                inherit output;
          ...
          lib.mkOptionDefault {
            ...
            keybindings = containerKeybinds // workspaceKeybinds // globalKeybinds
              // controlKeybinds;
